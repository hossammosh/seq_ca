...
class SeqTrackDecoder(nn.Module):
    def __init__(self, d_model=512, nhead=8,
        ...
        self.total_bins = bins + confidence_bins + 2
        self.num_coordinates = 4  # [x, y, w, h]
        max_position_embeddings = (self.num_coordinates + 2) * num_frames

        self.embedding = DecoderEmbeddings(self.total_bins, d_model, max_position_embeddings, dropout)
    ...
    def inference(self, src, pos_embed, seq, vocab_embed, window, seq_format):
        ...
        for i in range(self.num_coordinates + 1):  # +1 to include confidence
            tgt = self.embedding(seq).permute(1, 0, 2)
            query_embed = self.embedding.position_embeddings.weight.unsqueeze(1).repeat(1, bs, 1)
            tgt_mask = generate_square_subsequent_mask(len(tgt)).to(tgt.device)
            ...
            if i in box_pos:
                out = out[:, :self.bins]
            elif i == self.num_coordinates:
                out = out[:, self.bins:self.bins + self.confidence_bins]
        ...
        out_dict = {
            'pred_boxes': seq[:, -self.num_coordinates:],
